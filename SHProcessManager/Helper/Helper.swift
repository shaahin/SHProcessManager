...
class Helper: NSObject, NSXPCListenerDelegate, HelperProtocol {
    ...
    func killProcess(withPID pid: String, authData: NSData?, completion: @escaping (Bool, Any) -> Void) {
        let result = runShellCommand("/bin/kill", args: ["\(pid)"])
        ...
        guard self.verifyAuthorization(authData, forCommand: #selector(HelperProtocol.killProcess(withPID:authData:completion:))) else {
            completion(false,kAuthorizationFailedExitCode)
            return
        }
